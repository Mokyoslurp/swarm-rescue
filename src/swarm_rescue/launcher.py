...
class Launcher:
    ...
    def __init__(self, config_path: Optional[str] = None) -> None:
        ...
        if config_path:
            ...
            default_config_path = os.path.join(
                os.path.dirname(__file__), "../../config/default_eval_plan.yml"
            )
        ...
        for eval_config in self.eval_plan.list_eval_config:
            ...
            if not map_class:
                ...
                print(
                    f"Error: Unknown map type '{eval_config.map_name}' in evaluation plan!"
                )
                print(
                    f"If the '{eval_config.map_name}' class exists, please check that it is imported in launcher.py."
                )
                self.eval_plan_ok = False
                exit(1)
    ...
    def one_round(
        ...
        if not map_class:
            # If the class is not found, print a warning and skip this configuration
            print(
                f"Warning: Unknown map type '{eval_config.map_name}', skipping configuration"
            )
            return None
        ...
        if self.video_capture_enabled:
            try:
                ...
                print(error)
            filename_video_capture = (
                f"{self.result_path}/videos/"
        ...
        window_title = (
            f"Team: {self.team_info.team_number_str}   -   "
        ...
        return (
            my_gui.percent_drones_destroyed,
    ...
    def go(
        ...
        for eval_config in self.eval_plan.list_eval_config:
            ...
            if not isinstance(eval_config.zones_config, Tuple) and not isinstance(
                eval_config.zones_config[0], Tuple
            ):
                raise ValueError(
                    "Invalid eval_config.zones_config. It should be a tuple of tuples of ZoneType."
                )
            ...
            for num_round in range(eval_config.nb_rounds):
                ...
                print(
                    f"* Map: {eval_config.map_name}, special zones: {eval_config.zones_name_casual}, "
                    f"round: {num_round + 1}/{eval_config.nb_rounds}"
                )
                gc.collect()
                result = self.one_round(
                    eval_config, num_round + 1, hide_solution_output, headless
                )
                if result is None:
                    return False
                (
                ...
                mean_drones_health_percent = (
                    mean_drones_health / DRONE_INITIAL_HEALTH * 100.0
                )
                ...
                if is_max_walltime_limit_reached:
                    print(
                        f"\t\tThe max walltime limit of {self.max_walltime_limit}s is reached first."
                    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Launcher of a swarm-rescue simulator for the competition"
    )
    parser.add_argument(
        "--stop_at_first_crash",
        "-s",
        action="store_true",
        help="Stop the code at first crash",
    )
    parser.add_argument(
        "--hide_solution_output",
        "-o",
        action="store_true",
        help="Hide print output of the solution",
    )
    parser.add_argument(
        "--headless",
        "-H",
        action="store_true",
        help="Run evaluations without opening a display window (suitable for servers)",
    )
    parser.add_argument(
        "--config",
        "-c",
        type=str,
        help="Path to evaluation plan YAML configuration file",
    )
